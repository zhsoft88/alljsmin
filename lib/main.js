...
function run() {
  ...
  if (FS.existsSync(config_file)) {
    ...
    if (config.tag_file) {
      tag_file = PATH.join(input_dir, config.tag_file)
      if (!FS.existsSync(tag_file))
        error_exit(`file not exists: ${tag_file}`)

      parse_result = parse_tag_file(tag_file)
    }
  }
  ...
  if (alljs_list && alljs_file) {
    ...
    for (const file of alljs_list) {
      ...
      if (!found_is_debug && contents.includes(is_debug)) {
        found_is_debug = true
